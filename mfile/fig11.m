%%% codes for producing fig.12 in the following manuscript
...
% ------------------------------------
% ---------- NOTE 1:
% the following codes is for part (d) of figure 11, for other parts just
% the variable "x_S" in line 123 should be changed as follows:

% (a) x_S= A_S * sin(w_S * t);
% (b) x_S= A_S * sin(w_S * t)+1*sin(10 * t)+1*sin(13 * t);
% (c) x_S= A_S * sin(w_S * t)+1*sin(8 * t)+1*sin(13 * t);
% (d) x_S= A_S * sin(w_S * t)+0.2*sin(8 * t)+1*sin(13 * t);

% ---------- NOTE 2: this code is for a genral model and some parts of it do not relate to this figure
...
%-------------------------------------------
BR_2V_1V=1.5;  % coupling weight between units visual 2 and visual 1
% ------------------------------------------
t_stim=0;
t_stim_pres=0;
t_ISI=0;
...
t = -0.01;
T_present=70;  %stimulus presentation time
...
%%%%&&&
    k=0;
    %   input
    x_S= A_S * sin(w_S * t)+0.2*sin(8 * t)+1*sin(13 * t);
    input(i)=x_S;
