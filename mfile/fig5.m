%%% codes for producing fig.5 in the following manuscript
...
%%%
tPLOT=1024;
nfft = 2^nextpow2(tPLOT);
Fs=1/dt;
L=tPLOT;
f = Fs/2*linspace(0,1,nfft/2+1)*2*pi;
%%%
Pxx = fft(input(2000:2000+tPLOT),nfft)/L;
plot(f,2*abs(Pxx(1:nfft/2+1)),'r') 
Pxx = fft(out_1_V(2000:2000+tPLOT),nfft)/L;
hold on
plot(f,2*abs(Pxx(1:nfft/2+1)),'b') 
Pxx = fft(out_2_V(2000:2000+tPLOT),nfft)/L;
plot(f,2*abs(Pxx(1:nfft/2+1)),'k') 
xlim([1 12])
set(gca,'FontSize',10)
ylabel('PSD')
...
subplot(2,1,2);
Pxx = fft(input(2000+3*tPLOT:2000+4*tPLOT),nfft)/L;
plot(f,2*abs(Pxx(1:nfft/2+1)),'r') 
Pxx = fft(out_1_V(2000+3*tPLOT:2000+4*tPLOT),nfft)/L;
hold on
plot(f,2*abs(Pxx(1:nfft/2+1)),'b') 
Pxx = fft(out_2_V(2000+3*tPLOT:2000+4*tPLOT),nfft)/L;
plot(f,2*abs(Pxx(1:nfft/2+1)),'k') 
xlim([1 12])
xlabel('Frequency')
ylabel('PSD')
...
legend({'Stimulus','Unit 1','Unit 2'},'Location','Best','FontSize',8)
